...
const DINE_CATEGORIES = [
  ...
  { id: 'cafe', name: 'Artisan Cafes', icon: <FaCoffee className="w-3.5 h-3.5" /> },
  { id: 'thakali', name: 'Authentic Thakali', icon: <FaUtensils className="w-3.5 h-3.5" /> },
  { id: 'restaurant', name: 'Family & Italian', icon: <FaPizzaSlice className="w-3.5 h-3.5" /> },
  { id: 'fast-food', name: 'Fast Bites & Burgers', icon: <FaHamburger className="w-3.5 h-3.5" /> },
  { id: 'bakery', name: 'Gelato & Bakery', icon: <FaIceCream className="w-3.5 h-3.5" /> },
];
...
export default function DinePage() {
  ...
  const filteredStores = useMemo(() => {
    ...
    if (activeCategory === 'thakali') {
      return diningStores.filter((s) => s.name.toLowerCase().includes('thakali') || s.tags?.some((t) => t.toLowerCase().includes('thakali')));
    }
    if (activeCategory === 'bakery') {
      return diningStores.filter((s) => s.categorySlug === 'cafe' || s.tags?.some((t) => t.toLowerCase().includes('bakery') || t.toLowerCase().includes('gelato')));
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 flex flex-col" style={{ fontFamily: "'Montserrat', sans-serif" }}>
      ...
      <PageHeader
        title="Dine & Taste"
        subtitle="From authentic Himalayan Mustang Thakali and organic single-origin coffee to gourmet thin-crust pizza and international food courts, indulge your senses."
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16 space-y-20">
        ...
        {/* ========================================================================= */}
        {/* SECTION 3: FOOD COURT & PANORAMIC TERRACE SPOTLIGHT */}
        {/* ========================================================================= */}
        <section className="bg-gradient-to-r from-gray-900 via-[#1e1316] to-gray-900 text-white rounded-3xl p-8 sm:p-10 md:p-12 relative overflow-hidden shadow-xl border border-gray-800">
          ...
          <div className="relative z-10 max-w-3xl space-y-4">
            <div className="inline-flex items-center space-x-2 text-xs font-bold tracking-widest text-red-400 uppercase">
              <FaUtensils className="w-3.5 h-3.5" />
              <span>4th Floor Terrace Dining</span>
            </div>
            <h2
              className="text-2xl sm:text-3xl md:text-4xl font-bold tracking-wide leading-tight"
              style={{ fontFamily: "'Arizona Flare', 'Times New Roman', serif" }}
            >
              The 4th Floor Food Court & Outdoor Terrace
            </h2>
            <p className="text-sm sm:text-base text-gray-300 leading-relaxed font-light">
              Enjoy expansive dining with sweeping panoramic views of Pokhara valley and the snow-capped Annapurna range. Choose from 12+ live food stations offering Thakali thalis, sizzling sizzlers, momos, rolls, and refreshing milkshakes under one spacious roof.
            </p>
